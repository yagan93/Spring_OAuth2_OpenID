...
public class AuthorizationServerConfig {
    ...
    public OAuth2TokenCustomizer<JwtEncodingContext> jwtCustomizer() {
        return context -> {
            if (context.getTokenType().getValue().equals(OAuth2TokenType.ACCESS_TOKEN.getValue())) {
                ...
                context.getClaims().claim(AUTHORITIES_CLAIM_IDENTIFIER, authorities);
            }
        };
    }
    ...
    // Endpoint "oauth2/jwks" to expose web key set
    // Endpoint ".well-known/openid-configuration" to expose configuration
    @Bean
